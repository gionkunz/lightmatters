...
The root `AppComponent` SHALL inject `ThemeService` and ensure the document root theme class or attribute is applied during application initialization, before or as the landing page first renders.
...
`app.routes.ts` SHALL define a default route that lazy-loads `landingRoutes` from `@lightmatters/feature-landing` and a wildcard redirect to `/`.
...
- **WHEN** a file in `libs/features/landing` imports from `@lightmatters/design`
