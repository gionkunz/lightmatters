...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-3">
      ...
      <p
        class="m-0 font-serif text-[length:var(--lm-text-button-sm)] leading-snug text-ink"
      >
        {{ question() }}
      </p>
      <div class="mt-1 flex flex-col gap-2.5">
        @for (option of options(); track option.id) {
          <button
            type="button"
            lmInteractive
            class="cursor-pointer border px-4 py-3 text-left font-serif text-[length:var(--lm-text-button)] italic leading-snug transition-colors duration-200"
