...
import { STEP_01_TWO_WORLDLINES } from './step-01-two-worldlines';

@Component({
  selector: 'lm-ch8-twin-step-01',
  imports: [
    LmStepFrameComponent,
    LmDiagramViewportComponent,
    LmNarratorChatFeedComponent,
...
export class Step01Component implements OnInit, OnDestroy {
  ...
  protected readonly step = STEP_01_TWO_WORLDLINES;
  ...
  constructor() {
    this.registry.register('twin.progress', {
      get: () => this.progress(),
      set: (v) => this.progress.set(v),
      initial: 0,
    });
    this.runner = new TimelineRunner(this.step.timeline, this.registry);
    registerFeedbackStepContext(this.runner, this.chapterRoute, 1);
