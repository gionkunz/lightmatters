...
import { STEP_02_TWO_LISTENERS } from './step-02-two-listeners';

@Component({
  selector: 'lm-ch3-step-02',
  imports: [
    LmStepFrameComponent,
    LmDiagramViewportComponent,
        LmNarratorChatFeedComponent,
        ...
        <div class="flex min-h-0 flex-col">
          ...
          >
            <div class="mb-3.5 flex items-baseline justify-between gap-4">
              <lm-kicker [opacity]="0.55">two observers · symmetric</lm-kicker>
            </div>
...
export class Step02Component implements OnInit, OnDestroy {
  ...
  /** Scene time when the flash first reaches each symmetric observer. */
  private readonly receptionSceneTime = 0.6;

  protected readonly step = STEP_02_TWO_LISTENERS;
  ...
  protected readonly observers: LightSceneObserver[] = [
    ...
    { id: 'b', x: 0.6, y: 0, label: 'B', color: 'accent-2' },
  ];
  ...
  constructor() {
    ...
    registerFeedbackStepContext(this.runner, 4, 2);
    this.totalDurationMs = this.runner.getTotalDurationMs();

    effect(() => {
      if (this.time() < this.receptionSceneTime - RECEPTION_SOUND_WINDOW) {
