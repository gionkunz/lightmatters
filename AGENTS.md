...
Light Matters has an Nx integrated monorepo with the `lightmatters` Angular 21 app, foundational libraries (`libs/design`, `libs/engine`, `libs/physics`, `libs/primitives/spacetime-diagram`, `libs/primitives/light-scene`, `libs/primitives/curved-surface`), a design system + landing page at `/`, Chapter 1 Steps 1–4 at `/chapter/1/step/{1..4}`, Chapter 2 Steps 1–3 at `/chapter/2/step/{1..3}`, Chapter 3 Steps 1–5 at `/chapter/3/step/{1..5}`, Chapter 4 Steps 1–5 at `/chapter/4/step/{1..5}`, Chapter 7 (Doppler) Steps 1–5 at `/chapter/7/step/{1..5}`, Chapter 10 (Rolling) Steps 1–4 at `/chapter/10/step/{1..4}`, Chapter 11 (Center) Steps 1–6 at `/chapter/11/step/{1..6}`, Chapter 12 (Light bending) Steps 1–7 at `/chapter/12/step/{1..7}`, and a Playwright `lightmatters-e2e` project. The app builds with SSG (build-time prerender) and deploys to Cloudflare Pages via GitHub integration (static output plus repo-root Pages Function `POST /api/feedback` backed by D1). Global feedback UI: `LmFeedbackFabComponent` in the app shell; step playback context via `registerFeedbackStepContext(runner, chapter, step)` in each step constructor (see `libs/engine/src/lib/feedback/register-feedback-step-context.ts`). The timeline engine, narrator, step chrome, spacetime-diagram, light-scene, and curved-surface (WebGL) primitives are implemented. Chapters 5, 6, 8, and 9 are reserved for the incoming SR content changes (`add-c-invariance-and-twin-paradox`, `add-light-clock-and-length-contraction`, `add-matter-and-emc2`). Next up: `physics-accuracy-pass`, then the new SR chapters.
